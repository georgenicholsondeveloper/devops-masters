...
namespace Commodity.Api.UnitTests
{
    public class Tests
    {
        ...
        [Test]
        public async Task GetCommoditiesCount_ShouldReturn200_AndCountInFormatMessage(){

            //Arrange

            var expectedCount = 3;

            _commodityServiceMock.Setup(x => x.GetCommoditiesCountAsync()).ReturnsAsync(expectedCount);

             var sut = new CommodityController(_commodityServiceMock.Object);

             //Act

             var response = sut.GetCommoditiesCount();
            ...
            {
                Assert.That(actionResult.StatusCode, Is.EqualTo(200));
                Assert.That(actionResult.Value, Is.InstanceOf<string>());
                Assert.That(actionResult.Value as string, Is.EquivalentTo($"Count of commodities: {expectedCount}"));
            }
        }

        [Test]
        public async Task GetCommoditiesCountAsync_ShouldReturnCount_CalculatedFromCommodities(){
            ...
            await using var context = new CommodityDbContext(options);
            ...
            var sut = new CommodityService(context);

            // Act
            var response = await sut.GetCommoditiesCountAsync();
